...
DEFAULT_AMOUNT_REPORT_DIR = PROJECT_ROOT / "outputs" / "reports" / "supplychain_pps5000_amount_only"
...
def main() -> int:
    ...
    ap.add_argument("--output-dir", default="", help=f"Output directory for stats (default: {DEFAULT_AMOUNT_REPORT_DIR})")
    ap.add_argument(
        "--recommended-output-dir",
        default="",
        help="Preferred new-layout output directory (e.g. outputs/reports/amount_analysis); used only when --output-dir is not set",
    )
    ...
    elif args.recommended_output_dir:
        output_dir = Path(args.recommended_output_dir)
    else:
        output_dir = DEFAULT_AMOUNT_REPORT_DIR
