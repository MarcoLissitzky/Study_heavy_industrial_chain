...
DEFAULT_METRICS_CSV = PROJECT_ROOT / "outputs" / "viz" / "supplychain_pps5000" / "supplychain_node_metrics.csv"
DEFAULT_REPORTS_DIR = PROJECT_ROOT / "outputs" / "reports" / "supplychain_pps5000"
...
def main() -> int:
    ...
    ap.add_argument("--metrics-csv", default="", help=f"Path to supplychain_node_metrics.csv (default: {DEFAULT_METRICS_CSV})")
    ap.add_argument("--reports-dir", default="", help=f"Output directory for reports (default: {DEFAULT_REPORTS_DIR})")
    ...
    if args.metrics_csv:
        ...
        metrics_resolution = "explicit_arg"
    else:
        inferred = PROJECT_ROOT / "outputs" / "viz" / base_dir.name / "supplychain_node_metrics.csv"
        if inferred.exists():
            metrics_csv = inferred
            metrics_resolution = "inferred_from_base_dir"
        else:
            metrics_csv = DEFAULT_METRICS_CSV
            metrics_resolution = "legacy_default"
    ...
    out_dir = Path(args.reports_dir) if args.reports_dir else DEFAULT_REPORTS_DIR
