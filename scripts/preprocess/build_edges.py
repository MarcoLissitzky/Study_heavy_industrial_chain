...
DEFAULT_PROCESSED_DIR = PROJECT_ROOT / "data" / "processed" / "supplychain_pps5000"
...
def main() -> int:
    ...
    ap.add_argument("--out-dir", default="", help=f"Output directory (default: {DEFAULT_PROCESSED_DIR})")
    ap.add_argument(
        "--processed-dir",
        default="",
        help=f"Alias of --out-dir for new layout (default: {DEFAULT_PROCESSED_DIR}); takes precedence over --out-dir when set",
    )
    args = ap.parse_args()
    ...
    if args.node_csv:
        ...
        node_resolution = "explicit_arg"
    else:
        base_node = base_dir / "node.csv"
        processed_node = out_dir / "node.csv"
        if base_node.exists():
            node_csv = base_node
            node_resolution = "base_dir_default"
        else:
            node_csv = processed_node
            node_resolution = "processed_fallback"
